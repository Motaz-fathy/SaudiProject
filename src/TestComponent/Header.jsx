  ...
  import EventInfo from "./Home/EventInfo";
                ...
                <div className="hidden lg:flex items-center gap-4  ">
                  {menuItems.map((menu, index) =>
                    <div key={index} className="relative group">
                      {/* Menu Button */}
                      <button className="text-white hover:text-[#3fdaab] text-[14px] transition duration-300 flex justify-center gap-2 items-center">
                        {menu.title}
                        <svg
                      ...
                      {/* Dropdown */}
                      <div className="absolute z-40 left-0 top-[20px] hidden group-hover:flex flex-col bg-white text-black hover:text-[#3fdaab] transition duration-300 shadow-lg rounded-md mt-2 w-40">
                        {menu.items.map((item, i) =>
                          <a
                            key={i}
                            href="#"
                            className="block px-2 py-2 hover:text-green-400 transition text-xs"
                          >
                            {item}
                          </a>
                        )}
                  ...
                  <div className="p-4 space-y-4">
                    {menuItems.map((menu, index) =>
                      <div key={index} className="">
                        ...
                        >
                          <span className="text-white hover:text-[#006E62] text-xs">
                            {menu.title}
                          </span>
                          <svg
                          ...
                          <div className="flex flex-col ">
                            {menu.items.map((item, i) =>
                              <Link
                                ...
                                className={`block px-2 py-2 text-xs text-white hover:text-[#006E62] transition-all duration-300 ${activeItem === i && "border-r border-[#006E62]"}`}
                              >
                                {item}
                              </Link>
                            )}
